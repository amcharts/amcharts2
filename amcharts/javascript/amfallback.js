AmCharts.AmFallback = AmCharts.Class({	
	...
	write:function(holder){
		...
		if(this.type == "line" || this.type == "column"){
			...
			this.chart = chart;
			this.chart.pathToImages = this.pathToImages;
			chart.dataProvider = this.data;
	...
	parseXMLNode: function (dataNode, dataNodeName, xmlNode, level) {
		...
		if (xmlNode) {
			...
			for (var i = 0; i < cnt; i++) {
				...
				if (node.childNodes.length == 0 && value == '') {
					...
					if (node.nodeType == 3 || node.nodeType == 4) { // text node or CDATA
						...
						if (node.nodeType == 1) {
							...
							if (node.attributes != undefined && node.attributes.length) {
	...
	parseValue:function(val, type){
		switch (type) {
			...
			case '{s}':
				...
				//val = val.replace(/\n/g, ''); // new lines (to fix, only from start and end)
				if (val.replace(/ /g, '') == "") {
					val = undefined;
				}
				break;
	...
	setGraphSettings:function(graph, s){
		...
		if(s.axis == "right"){
			graph.valueAxis = chart.valueAxes[1];
		}				
	...
	createCursor:function(){
		...
		if (ss != undefined) {
			if (AmCharts.toBoolean(ss.enabled, true)) {				
	...
	setExceptions:function(){
		...
		if(type == "bar"){
			...
			this.chart.dataProvider.reverse();
		}			
		var cs = this.settings.column;
		if(cs != undefined){
			var st = cs.type;
			if(st == "stacked"){
				st = "regular";	
			...
			if (st) {
				this.chart.valueAxes[0].stackType = st;
			}
		}
	},				
